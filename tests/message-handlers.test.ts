...
describe('Message Handlers', () => {
    ...
    describe('handleCheckDomain (FR-3.3, FR-3.5)', () => {
        ...
        it('CRITICAL: should return isWhitelisted flag correctly', () => {
            allowedDomains.add('scam.com');
            const sendResponse = vi.fn();

            handleMessage({ action: 'checkDomain', url: 'https://scam.com' }, sendResponse);

            // The response must include isWhitelisted for popup to display correctly
            expect(sendResponse).toHaveBeenCalledWith(expect.objectContaining({
                isScam: false,  // Whitelist overrides blacklist
                isWhitelisted: true,
                domain: 'scam.com'
            }));
        });
